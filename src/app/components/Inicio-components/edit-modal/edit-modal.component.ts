  ...
  ngOnInit(): void {
    this.userservice.obtenerUsuario(this.idusuario).subscribe((data)=>{
      this.miFormulario.patchValue({
        nombre: data.response.nombreCompleto,
        cargoid:data.response.cargo.cargoId.toString()
      });
    })
    this.cargoservice.obtenerCargos().subscribe((data)=>{
      this.opcionesCargos=data.response;
    })
  }
  ...
  enviarFormulario() {
    if (this.miFormulario.valid) {
      const usuario: Usuario = {
        ...
        idCargo: parseInt(this.miFormulario.value.cargoid)
      };
